...
const template_expression_1 = require("./template-expression");
class TemplateEngine {
        ...
        do {
            ...
            this.wsh.eachCell(cellRange, (cell) => {
                ...
                matches.forEach((rawExpression) => {
                    const tplExp = new template_expression_1.TemplateExpression(rawExpression, rawExpression.slice(2, -2));
    ...
    processBlockPipes(cellRange, cell, pipes, data) {
        ...
        const newRange = cell_range_1.CellRange.createFromRange(cellRange);
        let insertedRows;
        try {
            pipes.forEach((pipe) => {
                switch (pipe.pipeName) {
    ...
    blockPipeRepeatRows(cell, dataArray, countRows) {
        ...
        let sectionRange = new cell_range_1.CellRange(startRow, wsDimension.left, endRow, wsDimension.right);
    ...
    blockPipeTile(cell, dataArray, blockRows, blockColumns, tileColumns) {
        ...
        const blockRange = new cell_range_1.CellRange(+cell.row, +cell.col, +cell.row + blockRows - 1, +cell.col + blockColumns - 1);
        ...
        let tileColumn = 1, tileRange = cell_range_1.CellRange.createFromRange(blockRange);
        dataArray.forEach((data, idx, array) => {
            // Prepare the next tile
            if ((idx !== array.length - 1) && (tileColumn + 1 <= tileColumns)) {
                const nextTileRange = cell_range_1.CellRange.createFromRange(tileRange);
            ...
            if (idx !== array.length - 1) {
                ...
                if (tileColumn <= tileColumns) {
                    ...
                    tileRange = cell_range_1.CellRange.createFromRange(blockRange);
                }
            }
        });
